...
- **Invitaciones con correo propio:** la app crea la invitación (token aleatorio de 128 bits) y la función `send-invitation` envía por Resend un correo con datos de la base y un enlace `?invite=TOKEN`; quien lo abre inicia sesión o crea su cuenta (correo o Google) y la invitación se acepta sola con el mismo correo. Límites: 1 envío por minuto y 5 por invitación, 10 invitaciones por hora por persona (`invitation_emails.sql`, probado con `test_invitation_emails.sql`).
...
| `send-invitation` | — (a demanda) | Envía el correo propio de invitación a un segundo tutor ("Ana te invitó a cuidar a Greta"), armado en el servidor, con límites de envío |
